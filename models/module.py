...
class FeatureNet(nn.Module):
    def __init__(self, base_channels, num_stage=3, stride=4, arch_mode="unet"):
        ...
        self.conv00 = Conv2d(3, base_channels, (3, 7, 11), 1, dynamic=True,padding=1)
        self.conv01 = Conv2d(base_channels, base_channels, (3, 5, 7), 1, dynamic=True,padding=1)

        self.downsample1 = Conv2d(base_channels, base_channels*2, 3, stride=2, padding=1)
        self.conv10 = Conv2d(base_channels*2, base_channels*2, (3, 5), 1, dynamic=True,padding=1)
        self.conv11 = Conv2d(base_channels*2, base_channels*2, (3, 5), 1, dynamic=True, padding=1)

        self.downsample2 = Conv2d(base_channels*2, base_channels*4, 3, stride=2, padding=1)
        self.conv20 = Conv2d(base_channels*4, base_channels*4, (1, 3), 1, dynamic=True, padding=1)
        self.conv21 = Conv2d(base_channels*4, base_channels*4, (1, 3), 1, dynamic=True, padding=1)
...
class CostRegNet(nn.Module):
    def __init__(self, in_channels, base_channels, last_layer=True, full_res=False):
        ...
        if full_res:
            ...
            self.conv8 = nn.Sequential(Deconv3d(base_channels * 4, base_channels * 4, stride=2, padding=1, output_padding=1),
                                       Conv3d(base_channels*4, base_channels*4, padding=1))
            ...
            self.conv10 = nn.Sequential(Deconv3d(base_channels * 2, base_channels * 2, stride=2, padding=1, output_padding=1),
                                       Conv3d(base_channels*2, base_channels*2, padding=1))
            ...
            self.conv12 = nn.Sequential(Deconv3d(base_channels, base_channels, stride=2, padding=1, output_padding=1),
                                       Conv3d(base_channels, base_channels, padding=1))
        else:
            self.conv7 = Deconv3d(base_channels * 8, base_channels * 4, stride=2, padding=1, output_padding=1)
            self.conv8 = Deconv3d(base_channels * 4, base_channels * 4, padding=1)

            self.conv9 = Deconv3d(base_channels * 4, base_channels * 2, stride=2, padding=1, output_padding=1)
            self.conv10 = Deconv3d(base_channels * 2, base_channels * 2, padding=1)

            self.conv11 = Deconv3d(base_channels * 2, base_channels * 1, stride=2, padding=1, output_padding=1)
            self.conv12 = Deconv3d(base_channels * 1, base_channels * 1, padding=1)
    ...
    def forward(self, x):
        ...
        x = conv4 + self.conv8(self.conv7(x))
        x = conv2 + self.conv10(self.conv9(x))
        x = conv0 + self.conv12(self.conv11(x))
        if self.last_layer:
            x = self.prob(x)
        return x
...
class Refinement(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv3 = ConvBnReLU(in_channels=16, out_channels=1)
...
def unity_regression(prob_volume, depth_values, interval):
    ...
    wta_depth = torch.sum(prob_volume * depth_values, 1)
    # print(((1 - val)).size())
    # print(prob_volume.size())
    # print((1 - val).size())
    # print(interval.size())
    # print("+++++++++++++++++++++++++++++++")
    interval=interval[0]
...
def get_cur_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, uncertainty_map,min_depth,max_depth):
    ...
    new_interval = (ndepth * depth_inteval_pixel) / (ndepth - 1)
    depth_inteval_pixel=depth_inteval_pixel.squeeze(1).squeeze(1)
    depth_inteval_pixel=depth_inteval_pixel[0]
    depth_inteval_pixel=float(depth_inteval_pixel)
    # print("-------------------------------------------")
    # print((ndepth / 2 * uncertainty_map * depth_inteval_pixel * 1.5).size())
    # print((2 * uncertainty_map * depth_inteval_pixel * 1.5).size())
    # print(depth_inteval_pixel)
    # print(uncertainty_map.size())
    # print(cur_depth.size())
    # print("-------------------------------------------")
    ...
    new_interval=new_interval.squeeze(1).squeeze(1)
...
def Loss_strategy(pro_volume,depth_values,depth_min,depth_max,interval):
    eps = 1e-10
    ...
    depth=unity_regression(pro_volume_normal,depth_values_clamp,interval)
